...
export async function GET(request) {
  try {
    ...
    if (!kullanici_id) {
      return Response.json({ success: false, error: 'kullanici_id zorunlu' }, { status: 400 });
    }
    ...
    const { data: feedbacks, error: fbError } = await supabase
      .from('cpr_feedback')
      .select('id, urun_id, stil, claim, arsivlendi, nedenler, created_at')
    ...
    console.error('Rapor kullanıcı hatası:', err);
