...
function getDateRange(filtre) {
  ...
  switch (filtre) {
    ...
    case '3aylik': {
      // Rapor çekilen ayın dönemi
      const mevcutDonem = Math.floor(now.getMonth() / 3);
      if (mevcutDonem === 0) {
        // 1. Dönemdeyiz, gösterilecek dönem yok — önceki yılın 4. dönemi
        start = new Date(now.getFullYear() - 1, 9, 1, 0, 0, 0, 0);
        end   = new Date(now.getFullYear() - 1, 11, 31, 23, 59, 59, 999);
      } else {
        // Mevcut dönemden önceki tüm tamamlanmış dönemler
    ...
    case 'yillik': {
      // Rapor çekilen ay hariç, Ocak'tan bir önceki aya kadar
      const oncekiAy = now.getMonth() - 1;
      if (oncekiAy < 0) {
        // Ocak'tayız, önceki yılın tamamı
...
function getTrendLabels(filtre) {
  ...
  switch (filtre) {
    ...
    case 'haftalik': {
      ...
      return tumGunler.map((g, i) => i <= gunSirasi ? g : null).filter(Boolean);
    }
...
export async function GET(request) {
  try {
    ...
    const { data: feedbacks, error: fbError } = await supabase
      .from('cpr_feedback')
      .select('id, firma_id, kullanici_id, urun_id, stil, claim, arsivlendi, nedenler, created_at')
    ...
    const firmaFbMap = {};
    fb.forEach(f => {
      if (!firmaFbMap[f.firma_id]) firmaFbMap[f.firma_id] = { uretilen: 0, arsivlenen: 0 };
      firmaFbMap[f.firma_id].uretilen++;
      if (f.arsivlendi) firmaFbMap[f.firma_id].arsivlenen++;
    });

    const firma = (firmalar || []).map(f => {
      const mumessilSayisi = (kullanicilar || []).filter(k => k.firma_id === f.id).length;
      const fData   = firmaFbMap[f.id] || { uretilen: 0, arsivlenen: 0 };
      const oran    = fData.uretilen > 0 ? Math.round(fData.arsivlenen / fData.uretilen * 100) : 0;
      const oranStr = `${oran}%`;
      const badge   = oran >= 40 ? 'badge-green' : oran > 0 ? 'badge-amber' : 'badge-gray';
      return [f.firma_adi, mumessilSayisi, fData.uretilen, fData.arsivlenen, oranStr, badge];
    }).sort((a, b) => b[2] - a[2]);
    ...
    const mumessil = (kullanicilar || []).map(k => {
      ...
      const oranStr = `${oran}%`;
      const badge   = oran >= 40 ? 'badge-green' : oran > 0 ? 'badge-amber' : 'badge-gray';
      const firmaAdi = firmaMap[k.firma_id] || '—';
      return [k.ad_soyad, firmaAdi, kData.uretilen, kData.arsivlenen, oranStr, badge, sonAktiviteStr(kData.son)];
    }).sort((a, b) => b[2] - a[2]);
    ...
    console.error('Rapor admin hatası:', err);
